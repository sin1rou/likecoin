...
export const generateCoin = functions.https.onRequest((request,response)=>{
...
function makeTransaction(data:object){
    return admin.firestore().collection("transaction").add(data);
}

export const send = functions.https.onRequest((request, response)=>{
    if(!request.body){
        response.send(400);
    } else {
